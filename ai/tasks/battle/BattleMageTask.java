...
public class BattleMageTask extends PhantomAITask {
   public BattleMageTask(L2PcInstance ph) {
      super(ph);
   }
      ...
      try {
         ...
         if ((is_need_to_get_new_target || this.returnToAllowedLocation(this.getComebackDistanceL() + 800)) && !this.phantom.isInsideZone((byte)1)) {
            ...
            if (Config.DEBAG_LVL > 0) {
               System.out.println(SupportBardTask$BattleMageTask.y("\uea02顃Б噥搋겢뵛ュ䝀錾\uda33几䚰攽픠펰䨟㋩褄밙\u2e45꽨ퟙ♨焪"));
            }
            ...
            if (Config.DEBAG_LVL > 0) {
               System.out.println(SupportBardTask$BattleMageTask.y("\uea02顃Б噥搋겢뵛ュ䝀錾\uda33几䚰攽픠펰䨟㋩褄밙\u2e45꽨ퟚ♨焪"));
            }
            ...
            if ((target != null && Rnd.chance(20) && target.isInRange((L2PcInstance)this.phantom, 600) || target == null) && !this.phantom.isCastingNow() && !this.phantom.isMoving()) {
   ...
   public boolean doAction() {
      if (super.doAction()) {
         ...
         if (Config.DEBAG_LVL > 0) {
            System.out.println(SupportBardTask$BattleMageTask.y("糼j\u0cfd\ue4ee㬱琒\uf7feॡ퀧㵇\ue30a옹䷾沅킂\udbb2া빛㘉餤譔ꡞ셩䁳ぺ"));
         }

         if (!this.phantom.phantom_params.isLockedTargetHere()) {
            return false;
         } else {
            L2Character target = this.phantom.phantom_params.getLockedTarget();
            if (target == null) {
               ...
               if (Config.DEBAG_LVL > 0) {
                  System.out.println(SupportBardTask$BattleMageTask.y("糼j\u0cfd\ue4ee㬱琒\uf7feॡ퀧㵇\ue30a옹䷾沅킂\udbb2া빛㘉餤譔ꡞ셮䁳ぺ"));
               }
               ...
               if (!this.phantom.isInRange(target, 900) || !(target instanceof L2NpcInstance) && (!target.isPlayer() || target.getActingPlayer().getAppearance().isInvisible())) {
                  if (Config.DEBAG_LVL > 0) {
                     System.out.println(SupportBardTask$BattleMageTask.y("糼j\u0cfd\ue4ee㬱琒\uf7feॡ퀧㵇\ue30a옹䷾沅킂\udbb2া빛㘉餤譔ꡞ셬䁳ぺ뿰㘗\ue20a╜긗\u0a62厦葚狊乒ꄳ䆻惀") + target.getName());
                  }
                  ...
                  if (Config.DEBAG_LVL > 0) {
                     System.out.println(SupportBardTask$BattleMageTask.y("糼j\u0cfd\ue4ee㬱琒\uf7feॡ퀧㵇\ue30a옹䷾沅킂\udbb2া빛㘉餤譔ꡞ셯䁳ぺ뿧㘌\ue228╏긐\u0a50叡萟犆") + target.getName());
                  }
               ...
               if (Config.DEBAG_LVL > 0) {
                  System.out.println(SupportBardTask$BattleMageTask.y("糼j\u0cfd\ue4ee㬱琒\uf7feॡ퀧㵇\ue30a옹䷾沅킂\udbb2া빛㘉餤譔ꡞ셭䁳ぺ"));
               }
